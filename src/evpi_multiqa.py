...
def answer_model(post_out, ques_out, ans_out, labels, args):
	...
	pq_a_loss = T.mean(lasagne.objectives.binary_crossentropy(pqa_preds[0], labels[:,0]))
	for i in range(N):
		for j in range(N):
			if i == 0 and j == 0:
				continue
			post_ques_ans = T.concatenate([post_out, ques_out[i][j], ans_out[i][j]], axis=1)
			...
			pq_a_loss += T.mean(lasagne.objectives.binary_crossentropy(pqa_preds[i*N+j], labels[:,i]))
	
	post_ques_ans_dense_params = lasagne.layers.get_all_params(l_post_ques_ans_dense, trainable=True)
	print 'Params in answer model ', lasagne.layers.count_params(l_post_ques_ans_dense)

	return pqa_preds, pq_a_loss, post_ques_ans_dense_params
...
def build(word_embeddings, len_voc, word_emb_dim, args, freeze=False):
	...
	pqa_preds, pq_a_loss, post_ques_dense_params = answer_model(post_out, ques_out, ans_out, labels, args)
	...
	train_fn = theano.function([posts, post_masks, ques_list, ques_masks_list, ans_list, ans_masks_list, labels], \
									[loss, pq_a_loss, pa_loss] + pqa_preds + pa_preds, updates=updates)
	test_fn = theano.function([posts, post_masks, ques_list, ques_masks_list, ans_list, ans_masks_list, labels], \
									[loss, pq_a_loss, pa_loss] + pqa_preds + pa_preds,)
		...
		for j in range(args.batch_size):
			...
			for k in range(N):
				if args.model == 'evpi_max':
					all_preds = [0.0]*N
					for m in range(N):
						all_preds[m] = math.exp(-_lambda*pqa_preds[j][k*N+m]) * pa_preds[j][k*N+m]
					preds[k] = max(all_preds)
				if args.model == 'evpi_sum':
					for m in range(N):
						preds[k] += math.exp(-_lambda*pqa_preds[j][k*N+m]) * pa_preds[j][k*N+m]
